...
- Both READMEs now document the loading bar's own BEM classes, and a new _Right-to-left_ section explains that the `indeterminate` sweep reverses under `[dir='rtl']` so it travels with the text. Both have worked since 22.1.0 with nothing in the documentation to find them by, which is the same as not having them.
